...
int main() {
    ...
    // Load variables from host to GPU memory
    float * voxel_grid_TSDF_toGPU;
    float * voxel_grid_weight_toGPU;

    // allocate memory in the device for weights and distance of TSDF grid
    cudaMalloc(&voxel_grid_TSDF_toGPU, voxel_grid_dim_x * voxel_grid_dim_y * voxel_grid_dim_z * sizeof(float));
    cudaMalloc(&voxel_grid_weight_toGPU, voxel_grid_dim_x * voxel_grid_dim_y * voxel_grid_dim_z * sizeof(float));

    // copy distance and weights from host(CPU) to device(GPU) to previously allocated memory above
    cudaMemcpy(voxel_grid_TSDF_toGPU, voxel_grid_TSDF, voxel_grid_dim_x * voxel_grid_dim_y * voxel_grid_dim_z * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(voxel_grid_weight_toGPU, voxel_grid_weight, voxel_grid_dim_x * voxel_grid_dim_y * voxel_grid_dim_z * sizeof(float), cudaMemcpyHostToDevice);

    float * intrinsic_matrix_toGPU;
    float * cam2base_toGPU;
    float * depth_im_toGPU;

    cudaMalloc(&intrinsic_matrix_toGPU, 3 * 3 * sizeof(float));
    cudaMemcpy(intrinsic_matrix_toGPU, intrinsic_matrix, 3 * 3 * sizeof(float), cudaMemcpyHostToDevice);
    cudaMalloc(&cam2base_toGPU, 4 * 4 * sizeof(float));
    cudaMalloc(&depth_im_toGPU, im_height * im_width * sizeof(float));
    ...
    for(int frame_idx = first_frame_idx; frame_idx < first_frame_idx + (int)num_frames; frame_idx++) {
        ...
        cudaMemcpy(cam2base_toGPU, cam2base, 4 * 4 * sizeof(float), cudaMemcpyHostToDevice);
        cudaMemcpy(depth_im_toGPU, depth_im, im_height * im_width * sizeof(float), cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(voxel_grid_TSDF, voxel_grid_TSDF_toGPU, voxel_grid_dim_x * voxel_grid_dim_y * voxel_grid_dim_z * sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(voxel_grid_weight, voxel_grid_weight_toGPU, voxel_grid_dim_x * voxel_grid_dim_y * voxel_grid_dim_z * sizeof(float), cudaMemcpyDeviceToHost);
