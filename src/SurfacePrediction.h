...
namespace internal{
    ...
    float interpolate_trilinearly(const Vector3f& point, const float* volume, const Vector3f& volume_size, const float voxel_scale){
        Vector3f point_in_grid = point.cast<int>();
    ...
    void raycast_tsdf_kernel(float* tsdf_volume,
        ...
        float tsdf = static_cast<float>(tsdf_volume.ptr(
                __float2int_rd(grid(2)) * volume_size.y() + __float2int_rd(grid(1)))[__float2int_rd(grid(0))].x) * DIVSHORTMAX;
        ...
        for(;ray_length < max_search_length; ray_length += truncation_distance * 0.5f){
            ...
            if (previous_tsdf > 0.f && tsdf < 0.f) {
                ...
                g_vertex.ptr(y)[x] = Vector3f(vertex.x(), vertex.y(), vertex.z());
                g_normal.ptr(y)[x] = Vector3f(normal.x(), normal.y(), normal.z());
    ...
    void surface_prediction(const VolumetricGridCuda& volume,
